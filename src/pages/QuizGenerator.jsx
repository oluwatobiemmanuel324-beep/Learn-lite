...
export default function QuizGenerator() {
    ...
    const fetchGroup = async () => {
      try {
        ...
        if (result.success && result.group) {
          ...
          localStorage.setItem('currentGroupId', String(result.group.id));
          localStorage.setItem('lastOpenedGroupId', String(result.group.id));
          if (result.group.joinCode) {
  ...
  useEffect(() => {
    ...
    handleGenerateQuizShortcut(location.state?.generationOptions || {});
  ...
  const loadGroupMessages = async () => {
    ...
    try {
      console.log('[LearnLite Chat] loadGroupMessages:start', { groupId, currentUserId });
      const result = await chatAPI.getMessages(groupId);
      if (!result?.success || !Array.isArray(result?.messages)) {
        console.log('[LearnLite Chat] loadGroupMessages:unexpected-response', result);
        return;
      }
      ...
      const normalizedMessages = result.messages.map((message) => {
        ...
        const tone = senderId && senderId === currentUserId
          ? 'outgoing'
          : (title && title.toLowerCase() === String(currentUsername || '').toLowerCase() ? 'outgoing' : 'incoming');
      ...
      console.log('[LearnLite Chat] loadGroupMessages:success', { count: normalizedMessages.length });
      setWorkspaceMessages(normalizedMessages);
    } catch (err) {
      console.log('[LearnLite Chat] loadGroupMessages:error', err);
  ...
  const handleSendMessage = async () => {
    console.log('[LearnLite Chat] submit:start', { draftMessage, groupId, isStandaloneMode });
    if (!draftMessage.trim()) {
      console.log('[LearnLite Chat] submit:aborted-empty-message');
      return;
    }
    ...
    if (isStandaloneMode) {
      console.log('[LearnLite Chat] submit:standalone-mode');
      appendWorkspaceMessage({
        tone: 'outgoing',
      ...
      setDraftMessage('');
      setReplyTarget(null);
      console.log('[LearnLite Chat] submit:standalone-finish');
      return;
    }

    try {
      console.log('[LearnLite Chat] submit:api-sendMessage:start');
      const sendResult = await chatAPI.sendMessage(groupId, {
        text: messageText,
        replyTo: outgoingReply
      });
      console.log('[LearnLite Chat] submit:api-sendMessage:response', sendResult);
      ...
      if (mentionsAI) {
        ...
        try {
          ...
          console.log('[LearnLite Chat] submit:ai-chat:error', aiErr);
      ...
      console.log('[LearnLite Chat] submit:error', err);
  ...
  const handleGenerateQuizShortcut = (generationOptions = null) => {
    if (!attachedNote) {
      alert('Upload a note first so the workspace knows what to generate from.');
      return;
    }

    const requestedCount = Math.max(5, Math.min(30, Number(generationOptions?.questionCount) || 10));
    const requestedDifficulty = ['easy', 'mixed', 'hard'].includes(String(generationOptions?.difficulty || '').toLowerCase())
      ? String(generationOptions?.difficulty).toLowerCase()
      : 'mixed';
    const requestedMode = String(generationOptions?.mode || 'exam-ready').toLowerCase();
    const shouldStartExamMode = Boolean(generationOptions?.startInExamMode || location.state?.startInExamMode);
    ...
    (async () => {
      try {
        ...
        const effectiveQuestions = parsedQuestions.length
          ? parsedQuestions
          : generateFallbackQuestionsFromNote(attachedNote.name, requestedCount);
          ...
          <div className="workspace-composer-wrap">
            ...
            <div className="workspace-composer">
              ...
              <textarea
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter' && !event.shiftKey) {
                    ...
                    console.log('[LearnLite Chat] input:onKeyDown-enter-send');
                    handleSendMessage();
                  }
                }}
              ...
              <button
                ...
                onClick={() => {
                  console.log('[LearnLite Chat] send-button:onClick');
                  handleSendMessage();
                }}
